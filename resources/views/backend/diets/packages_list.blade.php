...
<div class="page-header">
    <h1>
        <!-- {{ isset($data["Blang"]["Blang"]["Gift List"]) ? $data["Blang"]["Blang"]["Gift List"] : "礼包列表" }}  -->
        Packages List
        <!-- <small>
...
<div class="row">
	<div class="col-sm-2">
		<div class="form-group">
			<input type="text" class="form-control" name="product_name" value="{{ !empty('product_name') && request('product_name') ? request('product_name') : '' }}" placeholder='Search Product'>
		</div>
	</div>
        ...
        if(message == true){
	        $.ajax({
	           ...
	           success: function(response){
	                ...
	                window.location.href="{{ route('packages_list') }}";
	           },
	        });
	    }else{
	    	$('.loading-gif').hide();
