...
class DashboardController extends Controller
{
    public function index()
    {
    	...
    	$totalAgents = Merchant::select(DB::raw('COUNT(id) AS totalAgent'))
    						   ->where('status', '1')
                               ->first();
    	...
    	$agents = Merchant::where('status', '99')
    					  ->get();
    	...
    	return view('backend.dashboard.index', ['totalSales'=>$totalSales, 'totalCommission'=>$totalCommission, 'totalAgents'=>$totalAgents, 'totalProduct'=>$totalProduct,
    											'transactions'=>$transactions, 'agents'=>$agents, 'list'=>$list, 'list2'=>$list2, 'totalCustomer'=>$totalCustomer, 'totalAgent'=>$totalAgent]);
    }
