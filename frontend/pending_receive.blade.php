...
<div class="profile-content">
	<div class="container">
			...
			<div class="row justify-content-center">
				...
				<div class="col-2" align="center">
					<a href="{{ route('pending_shipping') }}" style="position: relative;">
						...
						<br>
						<span class="profile-word">{{ isset($data['lang']['lang']['to_be_delivered']) ? $data['lang']['lang']['to_be_delivered'] : 'To Ship' }}</span>
					</a>
				</div>
				...
				<div class="col-2" align="center">
					<a href="{{ route('pending_receive') }}" style="position: relative;">
						...
						<br>
						<span class="profile-word" style="font-weight: bold; text-decoration: underline;">{{ isset($data['lang']['lang']['to_be_received']) ? $data['lang']['lang']['to_be_received'] : 'To Receive' }}</span>
					</a>
				</div>
		...
		<div class="myOrder-list">
				...
				@if(isset($ship_details[$transaction->id]) && ($ship_details[$transaction->id] == 'Pending For Collection' || $ship_details[$transaction->id] == 'Collected' || $ship_details[$transaction->id] == 'Delivering(in transit)' ||
				$ship_details[$transaction->id] == 'Parcel Drop Off at Point') || $transaction->to_receive == 1)
				<div class="form-group container-box">
					<div class="row">
						...
						<div class="row">
							...
							<div class="col-sm-6">
								<div class="form-group product-details">
									...
									@if($transaction->status == 99)
										<span class="badge badge-pill badge-warning">{{ isset($data['lang']['lang']['unpaid']) ? $data['lang']['lang']['unpaid'] : 'Unpaid' }}</span>
									@elseif($transaction->status == 98)
										<span class="badge badge-pill badge-info">{{ isset($data['lang']['lang']['under_review']) ? $data['lang']['lang']['under_review'] : 'Waiting for verify' }}</span>
									@elseif($transaction->status == 97)
										<span class="badge badge-pill badge-info">{{ isset($data['lang']['lang']['under_review']) ? $data['lang']['lang']['under_review'] : 'Waiting for verify' }}</span>
									@elseif($transaction->status == 1)
										@if(!empty($transaction->bank_id))
				                            <span class="badge badge-pill badge-success">{{ isset($data['lang']['lang']['Paid']) ? $data['lang']['lang']['Paid'] : 'Paid' }}</span>
				                        @else
				                            <span class="badge badge-pill badge-success">{{ isset($data['lang']['lang']['Paid']) ? $data['lang']['lang']['Paid'] : 'Paid' }}</span>
				                        @endif
									@else
										<span class="badge badge-pill badge-danger">{{ isset($data['lang']['lang']['cancelled']) ? $data['lang']['lang']['cancelled'] : 'Cancelled' }}</span>
									@endif
								</div>
							</div>
					...
					@if($transaction->status == '1' && !empty($transaction->tracking_no) && !empty($transaction->order_number) && isset($ship_details[$transaction->id]))
					<hr>
					<div class="form-group">
